...
extern MY_HIGHP_OR_MEDIUMP vec2 spyware;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    if (tex.a < 0.001){tex.rgb = vec3(1.,0.,0.);}
    ...
    number light_strength = 0.4*(0.3*sin(2.*spyware.g) + 0.6 + 0.3*sin(spyware.r*3.) + 0.9);
    vec4 final_col;
    if (tex.a < 0.001){
        final_col = tex*colour + vec4(1., 0., .05,0.6)*light_strength*(1.+abs(offset_l)+abs(offset_r))*glow;
    }
    else{
        final_col = tex*colour + vec4(0.45, 0., 0.02,0.3)*light_strength*(1.+abs(offset_l)+abs(offset_r))*glow;
    }
